...
kproto_kPOD <- function(x, k, lambda = NULL, iter.max = 100, nstart = 1, na.rm = "yes", keep.data = TRUE, verbose = TRUE, ...){
  ...
  if(!is.data.frame(k)){
    if (length(k) == 1){
      ...
      # ========== MODIFICATION START ========== #
      # Allow initialization with incomplete cases
      complete_rows <- x[complete.cases(x),]
      if(nrow(complete_rows) >= k){
        ids <- sample(row.names(complete_rows), k)
      } else {
        # Fallback: Select rows with fewest NAs
        na_counts <- rowSums(is.na(x))
        ids <- order(na_counts)[1:k]
      }
      # ========== MODIFICATION END ========== #
  ...
  if(k > 1){
    ...
    for(l in 1:(k-1)){
      for(m in (l+1):k){
        # ========== MODIFIED CODE ========== #
        # Numeric distance (ignore NA)
        d1 <- sum(
          (protos[l, numvars, drop = FALSE] - protos[m, numvars, drop = FALSE])^2, 
          na.rm = TRUE  # Ignore NA
        )
        
        # Categorical distance (treat NA as mismatches)
        cat_diff <- 
          # Case 1: One is NA, the other is not
          (is.na(protos[l, catvars, drop = FALSE]) != is.na(protos[m, catvars, drop = FALSE])) | 
          # Case 2: Both not NA but different
          (protos[l, catvars, drop = FALSE] != protos[m, catvars, drop = FALSE] & 
             !is.na(protos[l, catvars, drop = FALSE]) & 
             !is.na(protos[m, catvars, drop = FALSE]))
        
        d2 <- sum(cat_diff)
        
        # Safe comparison (avoid NA)
        if (!is.na(d1 + d2) && (d1 + d2) == 0) {
          keep.protos[m] <- FALSE
        }
        # ================================== # 
      }
